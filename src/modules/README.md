...
Implemented routes are `/api/v1/health/live`, `/api/v1/health/ready`, `/api/v1/auth/login`, `/api/v1/auth/refresh`, and `/api/v1/auth/logout`. Another 143 historical V1 method/URL contracts are registered in `src/routes/legacy-v1-route-contracts.ts`; each currently returns HTTP 501 and is marked pending in OpenAPI. Legacy V3 handlers and tests are preserved in `reference/legacy-v3/`, but are not executed. Do not re-register those handlers against the V2 database; port each use case to the new schema first.
