...
export const riskManagementService = {
  ...
  async cancel(
    ...
    const cancelled = await riskManagementRepository.cancel(riskAssessmentId, input, current, {
      actorUserId: actor.userId,
      ...context,
    });
    if (!cancelled)
    ...
    return toRiskAssessmentDetail(cancelled);
  },
  ...
  async update(
    ...
    const score = input.likelihood * input.impact;
    const level = score <= 4 ? 'low' : score <= 9 ? 'medium' : score <= 16 ? 'high' : 'critical';
    const updated = await riskManagementRepository.update(
      riskAssessmentId,
      input,
      { score, level },
      current,
    ...
    return toRiskAssessmentDetail(updated);
  },
} as const;
