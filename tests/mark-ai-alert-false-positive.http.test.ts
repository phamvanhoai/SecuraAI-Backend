...
vi.mock('../src/modules/ai-anomaly-detection-alerts/ai-alerts.service.js', () => ({
  aiAlertsService: {
    list: vi.fn(), createFeedback: vi.fn(), listFeedback: vi.fn(), confirmAsIncident: vi.fn(),
...
describe('POST /api/v1/ai-alerts/:alertId/false-positive', () => {
  ...
  it('requires authentication', async () => {
    expect((await request(app).post(`/api/v1/ai-alerts/${alertId}/false-positive`)).status).toBe(401);
  });

  it('marks the alert as a false positive', async () => {
    vi.mocked(aiAlertsService.markFalsePositive).mockResolvedValue({
      id: alertId, alertCode: 'ALT-C82662FF', status: 'false_positive',
      reviewedByUserId: userId, reviewedAt: new Date('2026-09-26T00:00:00Z'), changed: true,
    });
