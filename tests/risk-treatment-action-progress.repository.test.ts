...
const action = () => ({
  ...
  risk_treatment_plans: {
    ...
    risk_assessment_id: '44444444-4444-4444-8444-444444444444',
    status: 'approved',
    completed_at: null,
    risk_assessments: {
      status: 'approved',
      ...
      residual_score: null as number | null,
    },
  },
});
const update = (progressPercent = 50, progressNote?: string) =>
  riskManagementRepository.updateTreatmentActionProgress(
    planId,
    actionId,
    {
...
describe('treatment action progress transaction', () => {
  beforeEach(() => {
    ...
    tx.risk_treatment_plans.findUnique.mockResolvedValue({
      risk_assessment_id: action().risk_treatment_plans.risk_assessment_id,
    });
    ...
    tx.risk_treatment_actions.aggregate.mockResolvedValue({
      _avg: { progress_percent: 50 },
      _count: { _all: 2 },
    });
  ...
  it.each([0, 50, 100])(
    'derives state from %i percent and audits in the transaction',
    async (percent) => {
      const result = await update(percent);
      expect(result.failure).toBeNull();
      const status = percent === 0 ? 'pending' : percent === 100 ? 'completed' : 'in_progress';
      expect(tx.risk_treatment_actions.updateMany).toHaveBeenCalledWith(
  ...
  it.each(['draft', 'rejected', 'pending_approval', 'completed', 'cancelled'])(
    'blocks plan state %s',
    async (status) => {
      const record = action();
      record.risk_treatment_plans.status = status;
  ...
  it.each(['draft', 'pending_approval', 'rejected', 'closed', 'cancelled'])(
    'blocks risk state %s',
    async (status) => {
      const record = action();
      record.risk_treatment_plans.risk_assessments.status = status;
      tx.risk_treatment_actions.findFirst.mockResolvedValue(record);
  ...
  it('blocks stale action versions before any write', async () => {
    tx.risk_treatment_actions.findFirst.mockResolvedValue({
      ...action(),
      updated_at: new Date(stamp.getTime() + 1),
    });
  ...
  it('requires an active actor and assignment or ownership', async () => {
    ...
    const record = action();
    record.assigned_to_user_id = 'other';
    record.risk_treatment_plans.owner_user_id = 'other';
    tx.risk_treatment_actions.findFirst.mockResolvedValue(record);
  ...
  it('rejects an action outside this plan and a cancelled action', async () => {
    ...
    expect(tx.risk_treatment_actions.findFirst).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('excludes cancelled actions, completes the plan, and keeps the risk open', async () => {
    tx.risk_treatment_actions.aggregate.mockResolvedValue({
      _avg: { progress_percent: 100 },
      _count: { _all: 2 },
    });
    ...
    expect(tx.risk_treatment_actions.aggregate).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('invalidates a legacy residual assessment when treatment progress changes', async () => {
    const record = action();
    record.risk_treatment_plans.status = 'in_progress';
    record.risk_treatment_plans.risk_assessments = {
      status: 'in_treatment',
    ...
    expect(tx.audit_logs.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ action: 'risk-assessment.residual-invalidated' }),
      }),
    );
  });
