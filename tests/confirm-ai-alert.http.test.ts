...
vi.mock('../src/modules/ai-anomaly-detection-alerts/ai-alerts.service.js', () => ({
  aiAlertsService: { list: vi.fn(), createFeedback: vi.fn(), listFeedback: vi.fn(), confirmAsIncident: vi.fn() },
}));
...
describe('POST /api/v1/ai-alerts/:alertId/confirm-incident', () => {
  ...
  it('requires authentication', async () => {
    expect((await request(app).post(`/api/v1/ai-alerts/${alertId}/confirm-incident`)).status).toBe(401);
  });

  it('confirms the alert and returns the incident', async () => {
    vi.mocked(aiAlertsService.confirmAsIncident).mockResolvedValue({
      id: alertId, alertCode: 'ALT-C82662FF', status: 'confirmed', reviewedByUserId: userId,
      reviewedAt: new Date('2026-09-25T00:00:00Z'), changed: true,
