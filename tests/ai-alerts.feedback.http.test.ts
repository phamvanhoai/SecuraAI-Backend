...
describe('AI alert reliability HTTP API', () => {
  beforeEach(() => {
    ...
    evaluateMock.mockResolvedValue({
      ai_feedback_id: 'feedback-1', ai_alert_id: alertId,
      reviewed_by_user_id: '00000000-0000-4000-8000-000000000001',
      feedback_label: 'needs_review', comment: null, created_at: new Date(),
    });
  });

  it('requires authentication and feedback permission', async () => {
    expect((await request(createApp()).post(`/api/v1/ai-alerts/${alertId}/feedback`).send({ feedbackLabel: 'needs_review' })).status).toBe(401);
    expect((await request(createApp()).post(`/api/v1/ai-alerts/${alertId}/feedback`).set('authorization', `Bearer ${token([])}`).send({ feedbackLabel: 'needs_review' })).status).toBe(403);
  ...
  it('records valid feedback and returns the standard response', async () => {
    ...
    expect(response.body).toMatchObject({ success: true, data: { alertId, feedbackLabel: 'needs_review' } });
  });
