...
describe('/api/v1/ai-alerts/:alertId/feedback', () => {
  ...
  it('creates validated feedback', async () => {
    vi.mocked(aiAlertsService.createFeedback).mockResolvedValue({
      id: userId, alertId, reviewedByUserId: userId, feedbackLabel: 'needs_review',
      comment: 'Investigate', createdAt: new Date('2026-09-25T00:00:00Z'),
    });
    ...
    expect(aiAlertsService.createFeedback).toHaveBeenCalledWith(
      userId, alertId, { feedbackLabel: 'needs_review', comment: 'Investigate' },
    );
  });
