...
Health routes, V2-backed authentication login, refresh and logout, protected `GET /users/me`, the Security Officer policy draft list, anomaly detection runs, the real-time AI alert feed, AI alert reliability feedback, and confirmation of alerts as incidents are currently implemented. Another 137 historical V1 method/URL contracts are registered in `src/routes/legacy-v1-route-contracts.ts` and return HTTP 501 until ported. Their V3 handlers and tests are in `reference/legacy-v3/` as non-running porting reference. Training is removed from both active code and the V3 porting reference; only immutable historical database snapshots may still mention it. Do not describe any pending business endpoint as implemented until its full route-to-repository path and tests exist.
