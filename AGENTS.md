...
Health routes, V2-backed authentication login, refresh and logout, protected `GET /users/me`, Security Officer policy draft listing and submission, Admin viewing of submitted policy drafts, anomaly detection runs, the real-time AI alert feed, AI alert reliability feedback, and confirmation of alerts as incidents are currently implemented. Another 135 historical V1 method/URL contracts are registered in `src/routes/legacy-v1-route-contracts.ts` and return HTTP 501 until ported. Their V3 handlers and tests are in `reference/legacy-v3/` as non-running porting reference. Training is removed from both active code and the V3 porting reference; only immutable historical database snapshots may still mention it. Do not describe any pending business endpoint as implemented until its full route-to-repository path and tests exist.
...
- Integrate branches through GitHub Pull Requests only: `feature/*` or `fix/*` into `develop`, then `develop` into `main`. Do not perform local merges and push the resulting merge commits directly to `develop` or `main`.
- When the user asks to merge, push only the source branch and create the appropriate Pull Request. If GitHub Pull Request tooling or authentication is unavailable, provide the compare URL and stop for the user to create/merge the PR; never bypass the PR workflow by pushing directly to an integration branch.
- Do not merge `develop` into a contributor's `feature/*` branch merely to update it before opening a Pull Request, and do not push agent-created synchronization commits to that contributor branch. If GitHub reports that the branch is mergeable, open the Pull Request from the contributor branch as-is.
- If a contributor branch conflicts with `develop`, preserve the contributor branch unchanged. Create a temporary conflict-resolution branch from the contributor branch (for example `integration/<feature>-to-develop`), merge the latest `origin/develop` into that temporary branch, resolve and test there, then open the Pull Request from the temporary branch into `develop`. Retain the contributor's original commits and authorship, and remove the temporary branch after the Pull Request is merged when branch deletion is authorized.
- Never push additional commits to another contributor's branch unless the user explicitly names that branch and authorizes modifying it. Test-only local worktrees do not grant permission to update the remote contributor branch.
- Treat `develop` and `main` as protected branches. Direct pushes are prohibited unless the user explicitly requests an emergency bypass and confirms the exact target branch after being warned that the action bypasses review and branch checks.
